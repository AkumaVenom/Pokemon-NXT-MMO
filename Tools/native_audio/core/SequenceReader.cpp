...
bool SequenceReader::TrackMain(MP2KPlayer &player, MP2KTrack &trk)
{
    ...
    while (trk.delay == 0) {
        if (++commandBudget > 100000) throw Xcept("Runaway MP2K track at {:#x}", trk.pos);
        trk.renderCommandPositions.try_emplace(trk.pos, ctx.renderSampleCursor, player.tickCount);
        ...
        if (cmd < 0x80) {
            cmd = trk.lastCmd;
            if (cmd < 0x80) {
                // NXT export must report damaged running status, never hide a missing track.
                throw Xcept("Uninitialized MP2K running status at {:#x}", trk.pos);
            }
        } else {
            trk.pos++;
            if (cmd >= 0xBD) {
...
void SequenceReader::cmdPlayCommand(MP2KPlayer &player, MP2KTrack &trk, uint8_t cmd)
{
    ...
    case 0xB2:
        // GOTO
        {
            const size_t target = rom.ReadAgbPtrToPos(trk.pos);
            auto it = trk.renderCommandPositions.find(target);
            if (it != trk.renderCommandPositions.end() && ctx.renderSampleCursor > it->second.first)
                ctx.renderLoops.push_back({trk.trackIdx, it->second.first, ctx.renderSampleCursor, it->second.second, player.tickCount});
        }
        if (trk.trackIdx == 0) {
    ...
    case 0xCE:
        ...
        break;
    default:
        throw Xcept("Unsupported MP2K command at {:#x}", trk.pos);
    }
}
