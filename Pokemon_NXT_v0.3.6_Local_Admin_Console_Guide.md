...
**Release:** gameplay **0.3.6-alpha**, build tools **1.4.0**. Based on the user-accepted 0.3.5 Pokémon Varieties source with the 1.3.4 Windows build fix. This is Pokémon NXT MMO, not Pokémon Vortex NXT. Proposal selection is recorded separately in `LOCAL_ADMIN_PROPOSAL_AUDIT.md` / `.json`.

## 1. Build, upgrade and start

Download **all four** `Pokemon_NXT_MMO_v0.3.6-alpha_Source_LocalAdminConsole_Part1.zip` through `Part4.zip`. Extract them into the **same new destination**, merging their identical `Pokemon_NXT_MMO_v0.3.6-alpha_Source_LocalAdminConsole` folders. They are ordinary ZIPs, not byte-split volumes: do not concatenate them. Run `BUILD_ALL.bat` only when all four are extracted. The banner should show **build 1.4.0 / gameplay 0.3.6-alpha**. All original game assets are included; no previous patch, ROM, images.rar, image renderer or audio extractor is needed.
