...
class Battle:
 def __init__(self,content,kind,players,names,rosters,items,turn_seconds=45):
 ...
 def usable(self,mon):
  return [i for i,m in enumerate(mon['moves']) if m['pp']>0 and (self.c.moves[str(m['id'])]['power']>0 or m['id'] in STATUS_MOVES or m['id'] in FIXED)]
 def choose(self,side,data):
  ...
  elif kind=='run':action={'action':kind}
 ...
 def timeout(self):
  ...
  for side in range(2):
   if side not in self.choice:self.choice[side]={'action':'run'}
  return True
 def view(self,side):
  return {'id':self.id,'kind':self.kind,'turn':self.turn,'you':self.c.public_mon(self.mon(side)),'opponent':self.c.public_mon(self.mon(1-side),False),'opponentName':self.names[1-side],'party':[self.c.public_mon(m) for m in self.rosters[side]],'opponentRemaining':sum(m['hp']>0 for m in self.rosters[1-side]),'waiting':side in self.choice,'seconds':max(0,int(self.deadline-time.monotonic())),'usable':self.usable(self.mon(side)),'log':self.logs[-18:],'ended':self.ended,'result':None if not self.ended else 'caught' if self.caught else 'won' if self.winner==side else 'lost' if self.winner is not None else 'escaped'}
 def _priority(self,s):
  a=self.choice[s];m=self.mon(s);priority={'run':10,'switch':6,'capture':5,'item':5}.get(a['action'],0)
 ...
 def resolve(self):
  require(len(self.choice)==2,'Waiting for both trainers.');self.protected=set();self.logs=[]
  for side in sorted((0,1),key=self._priority,reverse=True):
   if self.ended:break
   m=self.mon(side);enemy=self.mon(1-side);a=self.choice[side]
   if m['hp']<=0:continue
   k=a['action']
   if k=='run':self.ended=True;self.winner=1-side if self.kind=='duel' else None;self.logs.append(f'{self.names[side]} withdrew from battle.');break
   if k=='switch':self.active[side]=a['index'];self.logs.append(f'{self.names[side]} sent out {self.name(self.mon(side))}!');continue
   if k=='item':
    item=a['item'];self.items[side][item]-=1;old=m['hp'];m['hp']=min(self.c.stats(m)[0],m['hp']+self.c.items[item]['heal']);self.logs.append(f'{self.name(m)} recovered {m["hp"]-old} HP.');continue
   if k=='capture':
    ...
    if self.c.rng.random()<chance:self.caught=copy.deepcopy(enemy);self.caught['originalTrainer']=self.names[side];self.ended=True;self.winner=side;self.logs.append(f'Gotcha! {self.name(enemy)} was caught!');break
    self.logs.append(f'{self.name(enemy)} broke free!');continue
   if m['status']=='sleep':
    m['sleep']=max(0,m.get('sleep',1)-1)
    if m['sleep']>0:self.logs.append(f'{self.name(m)} is asleep.');continue
    m['status']='';self.logs.append(f'{self.name(m)} woke up!')
   if m['status']=='paralysis' and self.c.rng.random()<.25:self.logs.append(f'{self.name(m)} is fully paralyzed.');continue
   slot=a['slot'];mid=165 if slot==-1 else m['moves'][slot]['id'];move=self.c.moves[str(mid)]
   if slot>=0:m['moves'][slot]['pp']-=1
   self.logs.append(f'{self.name(m)} used {move["name"]}!')
   accuracy=move['accuracy'] or 100;accstage=self.tiers(m)[5]-self.tiers(enemy)[6];accuracy*=((3+accstage)/3 if accstage>=0 else 3/(3-accstage))
   if self.c.rng.randrange(100)>=accuracy:self.logs.append('The attack missed!');continue
   if enemy['uid'] in self.protected:self.logs.append(f'{self.name(enemy)} protected itself.');continue
   ...
   if mid==165:mult=1
   if mult==0:self.logs.append('It had no effect.');continue
   if mid in FIXED:
    fixed=FIXED[mid];damage=m['level'] if fixed==-1 else max(1,enemy['hp']//2) if fixed==-2 else fixed
   else:
    physical=typ<=8;ai,di=(1,2) if physical else (4,5);attack=self.c.stats(m)[ai]*stage(self.tiers(m)[ai]);defense=self.c.stats(enemy)[di]*stage(self.tiers(enemy)[di]);attack*=.5 if physical and m['status']=='burn' else 1
   ...
   enemy['hp']=max(0,enemy['hp']-damage);self.logs.append(f'{self.name(enemy)} lost {damage} HP.')
   if mult>1:self.logs.append("It's super effective!")
   elif mult<1:self.logs.append("It's not very effective.")
   if mid in (71,72,141,202):m['hp']=min(self.c.stats(m)[0],m['hp']+max(1,damage//2))
   if mid in (36,38,165):m['hp']=max(0,m['hp']-max(1,damage//4))
   ...
   if mid in secondary and enemy['hp']>0 and not enemy['status']:
    st,ch=secondary[mid]
    if self.c.rng.random()<ch:enemy['status']=st;self.logs.append(f'{self.name(enemy)} became {st}.')
  if not self.ended:
   for side in (0,1):
    m=self.mon(side)
    if m['hp']>0 and m['status'] in ('poison','burn','toxic'):
     damage=max(1,self.c.stats(m)[0]//8);m['hp']=max(0,m['hp']-damage);self.logs.append(f'{self.name(m)} is hurt by {m["status"]}.')
    if m['hp']>0 and m['uid'] in self.seeded:
     amount=min(m['hp'],max(1,self.c.stats(m)[0]//8));m['hp']-=amount;e=self.mon(1-side);e['hp']=min(self.c.stats(e)[0],e['hp']+amount);self.logs.append(f'Leech Seed drained {self.name(m)}.')
   for side in (0,1):
    if self.mon(side)['hp']<=0:
     self.logs.append(f'{self.name(self.mon(side))} fainted!');alive=next((i for i,m in enumerate(self.rosters[side]) if m['hp']>0),None)
     if alive is None:self.ended=True;self.winner=1-side
     else:self.active[side]=alive;self.logs.append(f'{self.names[side]} sent out {self.name(self.mon(side))}!')
  self.choice={};self.turn+=1;self.deadline=time.monotonic()+self.turn_seconds
 def _status(self,side,mid):
  m=self.mon(side);enemy=self.mon(1-side)
  changes={14:(True,1,2),28:(False,5,-1),39:(False,2,-1),43:(False,2,-1),45:(False,1,-1),74:(True,4,1),81:(False,3,-1),97:(True,3,2),106:(True,2,1),108:(False,5,-1),110:(True,2,1),111:(True,2,1)}
  if mid in changes:
   own,index,delta=changes[mid];target=m if own else enemy;t=self.tiers(target);old=t[index];t[index]=max(-6,min(6,old+delta));self.logs.append(f'{self.name(target)}: {"stat rose" if delta>0 else "stat fell"}.' if old!=t[index] else 'The stat cannot change further.');return
  statuses={77:'poison',78:'paralysis',79:'sleep',86:'paralysis',92:'toxic'}
  if mid in statuses:
   st=statuses[mid];types=self.c.species[enemy['species']]['types']
   if enemy['status'] or st in ('poison','toxic') and any(t in types for t in (3,8)) or mid==86 and 4 in types:self.logs.append('It had no effect.');return
   enemy['status']=st;enemy['sleep']=self.c.rng.randint(2,4) if st=='sleep' else 0;self.logs.append(f'{self.name(enemy)} became {st}.');return
  if mid==73:
   if 12 in self.c.species[enemy['species']]['types']:self.logs.append('It had no effect.')
   else:self.seeded.add(enemy['uid']);self.logs.append(f'{self.name(enemy)} was seeded.')
  elif mid in (105,135,208,235,236):m['hp']=min(self.c.stats(m)[0],m['hp']+self.c.stats(m)[0]//2);self.logs.append(f'{self.name(m)} recovered HP.')
  elif mid==156:m['hp']=self.c.stats(m)[0];m['status']='sleep';m['sleep']=3;self.logs.append(f'{self.name(m)} fell asleep and recovered HP.')
  elif mid==182:self.protected.add(m['uid']);self.logs.append(f'{self.name(m)} protected itself.')
  elif mid in (46,100):
   if self.kind=='wild':self.ended=True;self.winner=None;self.logs.append('The wild encounter ended.')
   else:self.logs.append('But it failed in this trainer battle.')
  elif mid==116:self.logs.append('Focus Energy is cosmetic in this alpha.')
  else:self.logs.append('But nothing happened!')
