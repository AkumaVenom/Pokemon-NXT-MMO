...
class Battle:
 ...
 def sendout_audio(self,side):
  mon=self.mon(side);self.audio('sendout',side,species=mon['species'])
 ...
 def timeout(self):
  ...
  return True
 def view(self,side):
  return {'id':self.id,'kind':self.kind,'source':self.audio_source,'turn':self.turn,'you':self.c.public_mon(self.mon(side)),'opponent':self.c.public_mon(self.mon(1-side),False),'opponentName':self.names[1-side],'party':[self.c.public_mon(m) for m in self.rosters[side]],'opponentRemaining':sum(m['hp']>0 for m in self.rosters[1-side]),'waiting':side in self.choice,'seconds':max(0,int(self.deadline-time.monotonic())),'usable':self.usable(self.mon(side)),'log':self.logs[-18:],'ended':self.ended,'result':self.result(side),'audio':self.audio_view(side)}
 ...
 def resolve(self):
  require(len(self.choice)==2,'Waiting for both trainers.');self.protected=set();self.logs=[];self.reset_audio()
  for side in sorted((0,1),key=self._priority,reverse=True):
   if self.ended:break
   m=self.mon(side);enemy=self.mon(1-side);a=self.choice[side]
   if m['hp']<=0:continue
   k=a['action']
   if k=='run':self.ended=True;self.winner=1-side if self.kind=='duel' else None;self.logs.append(f'{self.names[side]} withdrew from battle.');self.audio('escape',side);break
