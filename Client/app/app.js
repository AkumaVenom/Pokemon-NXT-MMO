...
function handle(p){
 ...
 switch(p.type){
  ...
  case 'state':if(state&&Number.isSafeInteger(p.revision)&&p.revision<state.revision)return;state=p;renderer.setCutTrees(p.adventure?.cutTrees);renderer.setStoryEvents(p.adventure?.storyEvents);updateParty();refreshOwnerModal();break;
...
function showBag(){
 ...
 for(const [key,item] of Object.entries(content.items)){
  const description=item.keyItem?(item.description||'Story Key Item'):item.heal?'Restores '+item.heal+' HP':item.capture?'Wild capture ball':item.evolutionStone?'Evolution item · Inspect a compatible Pokémon to use it':'Supplies';
  const actions=h('div',{class:'item-actions'});
  if(item.buyable!==false){actions.append(commandButton('Buy 1','buy',{item:key,quantity:1},'secondary',!nearMart||state.money<item.price,true),commandButton('Buy 5','buy',{item:key,quantity:5},'secondary',!nearMart||state.money<item.price*5,true));}
  else actions.append(h('span',{class:'pill'+(state.items[key]?' mint':'')},state.items[key]?'KEY ITEM OWNED':'KEY ITEM'));
  if(item.heal)actions.append(commandButton('Use on lead','use',{item:key,uid:state.party[0]},'secondary',!state.items[key]));
  body.append(h('article',{class:'item-row'},h('div',{},h('h3',{},item.name+' × '+(state.items[key]||0)),h('p',{},(item.keyItem?'Not for sale · ':money(item.price)+' each · ')+description)),actions));
 }
}
