...
export class WorldRenderer {
 ...
 resetSession(){++this.generation;this.pendingMap=null;this.players.clear();this.cutTrees.clear();this.storyEvents.clear();this.itemPickups.clear();this.map=null;this.selfId=null;this.hits=[];this.active=false;}
 ...
 setItemPickups(items){
  this.itemPickups=new Set(Array.isArray(items)?items.filter(id=>typeof id==='string'&&id.length<=96):[]);this.pruneHiddenObjectHits();
 }
 ...
 objectVisible(obj,mapId=this.map?.id){return !!obj&&!(obj.graphics===95&&this.cutTrees.get(mapId)?.has(obj.id))&&!(obj.storyEvent&&this.storyEvents.has(obj.storyEvent))&&!(obj.itemPickup&&this.itemPickups.has(obj.itemPickup));}
